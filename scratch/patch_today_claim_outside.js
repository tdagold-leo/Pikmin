...
// 1. Add markPostcardClaimedToday function
const funcCode = `
...
if (!mainJs.includes('function markPostcardClaimedToday')) {
    mainJs += '\n' + funcCode;
    console.log("Added markPostcardClaimedToday function");
}

// 2. Update sgDateBadge generation
const oldSgBadgeRegex = /let sgDateBadge = '';[\s\S]*?sgDateBadge \+= `<div style="color: #0369a1[\s\S]*?<\/div>`;\s*}\s*}\s*}/;

const newSgBadge = `let sgDateBadge = '';
...
if (oldSgBadgeRegex.test(mainJs)) {
    mainJs = mainJs.replace(oldSgBadgeRegex, newSgBadge);
    console.log("Patched sgDateBadge");
} else {
    console.log("sgDateBadge regex failed");
}
