...
// Update header subtitle and top button
const headerTarget = `<header>
                    <h1>Pikmin 自動註冊</h1>
                    <div class="subtitle">Cloud SPA 版・完全免費無超時</div>
                </header>
...
const headerReplacement = `<header>
                    <h1>Pikmin 自動註冊</h1>
                    <div class="subtitle">極速 1-Click 模式・自動複製信箱與系統推播驗證碼</div>
                </header>

                <div class="card">
                    <!-- 頂部極致 1-Click 快捷主按鈕 -->
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 22px;">
                        <button id="cloud-startBtn" class="btn-primary" style="margin: 0; border-radius: 14px; font-weight: 800; background: linear-gradient(135deg, #0284c7, #2563eb); box-shadow: 0 4px 18px rgba(37, 99, 235, 0.45); border: 1px solid rgba(255,255,255,0.2); font-size: 16px; padding: 16px 0; transition: all 0.2s; color: white; display: flex; align-items: center; justify-content: center; gap: 8px; cursor: pointer;">
                            ...
                            <span>🚀 產生並出發 (自動複製信箱 + 開啟遊戲)</span>
                        </button>
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0 4px;">
                            <a id="cloud-openPikminBtn" href="https://pikminbloom.onelink.me/pWSt/73s4bj4n" target="_blank" style="text-decoration: none; color: #10b981; font-size: 12px; font-weight: bold; display: flex; align-items: center; gap: 4px;">
                                🍄 手動開啟 Pikmin
                            </a>
                            <button id="cloud-enableNotifBtn" type="button" style="background: none; border: none; color: #38bdf8; font-size: 12px; cursor: pointer; display: flex; align-items: center; gap: 4px; padding: 0;">
                                🔔 開啟手機通知推播
                            </button>
                        </div>
                    </div>`;

if (indexContent.includes(headerTarget)) {
    indexContent = indexContent.replace(headerTarget, headerReplacement);
    fs.writeFileSync(indexFile, indexContent, 'utf-8');
    console.log('index.html updated successfully.');
} else {
    console.log('Header target string not matched in index.html');
}
...
const newIIFE = `// ===== 雲端自動註冊小幫手邏輯 =====
    (function() {
        ...
        async function runAutomation() {
            ...
            startBtn.innerHTML = '<span>⏳ 產生中並準備跳轉遊戲...</span>';
...
console.log('Successfully updated main.js with 1+2+3 streamlined workflow and target 4!');
